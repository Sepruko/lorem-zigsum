...
const help =
    ...
    \\
    \\Flags:
    \\  -f, --force Force memory allocations larger than 32KB (thirty-two kilobytes).
    \\
;
...
pub fn main() !u8 {
    ...
    var generated_lorem_ipsum = generator.generateLoremIpsum(allocator, amount, if (force) null else 1024 * 32) catch |err| switch (err) {
        error.BufferTooLarge => prompt_force: {
            var std_in = std.io.getStdIn().reader();

            try std_err.writeAll(
                "Refusing to allocate more than 32KB (thirty-two kilobytes) of memory, force generation? (y/N) ",
            );
            ...
            if (std.ascii.eqlIgnoreCase(read, "Y") or std.ascii.eqlIgnoreCase(read, "Yes"))
                break :prompt_force try generator.generateLoremIpsum(allocator, amount, null);

            try std_err.print("Cancelled printing {d} {s}.\n", .{
                amount,
                if (amount == 1) "word" else "words",
            });
            return 1;
        },
        else => {
            try std_err.print("Failed to generate the requested {d} {s}: {!}\n", .{
                amount,
                if (amount == 1) "word" else "words",
                err,
            });
